...
def luce_utility(time):
    if time>1:
        deltafunc = (zeta*(-buyselldiff[-1]))/(1+zeta*np.abs(buyselldiff[-1]))
        utility_list = list(map(lambda x :  w*((np.where(deltafunc>0,1,0))*( np.power(x,deltafunc))+(np.where(deltafunc<=0,1,0))*(1-np.power(x,-deltafunc))) + (1-w)*x*mu , luceset_))
        ...
        if sum(utility_list)!=0:
            ...
            chosen_item = np.random.choice(luceset_, p = 1/(len(luceset_)))
            return chosen_item
    else:
        return 0.5
...
def grid_update(size,grid,x, y,probability_array):
    ...
    probability_array[x][y] = luce_utility(time) + alphabuy*states[1] + alphasell*states[0]  
...
#then add randomness to price, look at black scholes to see how to set changing variance